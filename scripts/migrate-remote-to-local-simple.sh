#!/bin/bash
# Simple Data Migration Script: Remote to Local
# Uses wrangler d1 execute directly for SQL inserts

set -e

BACKUP_DIR="backups/remote-export-20251122-154410"

echo "🚀 Starting data migration from remote to local..."
echo ""
echo "Source: $BACKUP_DIR"
echo "Target: Local D1 database"
echo ""

# Function to extract and insert data from JSON export
insert_users() {
  echo "👥 Migrating users..."

  # User 1
  pnpm wrangler d1 execute chimera-d1 --local --command "
    INSERT OR IGNORE INTO users (
      id, role, updated_at, created_at, email,
      reset_password_token, reset_password_expiration,
      salt, hash, login_attempts, lock_until
    ) VALUES (
      1, 'creator', '2025-10-16T01:08:52.156Z', '2025-10-15T20:25:51.368Z', 'mike@the-ottoman.com',
      NULL, NULL,
      'fa14d866ffb86a01ad7d3fdb92768c19a3dc75574e2183d01649954208e82f09',
      '',
      0, NULL
    );
  " > /dev/null

  # User 2
  pnpm wrangler d1 execute chimera-d1 --local --command "
    INSERT OR IGNORE INTO users (
      id, role, updated_at, created_at, email,
      reset_password_token, reset_password_expiration,
      salt, hash, login_attempts, lock_until
    ) VALUES (
      2, 'admin', '2025-10-17T14:19:29.191Z', '2025-10-15T20:25:51.446Z', 'mike@luckbat.com',
      NULL, NULL,
      '7aae678915dbbaef3090204be712e0c68d48c3f8f1b3dfe7daf9d684e317a321',
      '',
      0, NULL
    );
  " > /dev/null

  echo "  ✅ Migrated 2 users"
}

echo "Migrating data..."
insert_users

echo ""
echo "🔍 Verifying migration..."
pnpm wrangler d1 execute chimera-d1 --local --command "
  SELECT 'users' as table_name, COUNT(*) as count FROM users
  UNION ALL SELECT 'comics', COUNT(*) FROM comics
  UNION ALL SELECT 'chapters', COUNT(*) FROM chapters
  UNION ALL SELECT 'pages', COUNT(*) FROM pages
  UNION ALL SELECT 'media', COUNT(*) FROM media;
"

echo ""
echo "✅ Users migrated successfully!"
echo "Note: This script only migrates users as a test. Run the full TypeScript migration for all data."
